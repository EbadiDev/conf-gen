...
show_usage() {
    echo "Usage:"
    echo "  Server: $0 server <name> <port> <default_token> <remote_public_key> <client_port> <tcp|udp> <nodelay>"
    echo "  Client: $0 client <name> <domain/ip:port> <default_token> <remote_public_key> <client_port> <tcp|udp> <nodelay>"
    echo ""
    echo "Examples:"
    echo "  $0 server myapp 2333 mysecrettoken GQYTKSbWLBUSZiGfdWPSgek9yoOuaiwGD/GIX8Z1kkE= 8080 tcp true"
    echo "  $0 client myapp example.com:2333 mysecrettoken cQ/vwIqNPJZmuM/OikglzBo/+jlYGrOt9i0k5h5vn1Q= 8080 tcp false"
    echo ""
    echo "Parameters:"
    ...
    echo "  remote_public_key - Remote peer's public key"
...
generate_keys() {
    print_info "Generating rathole keys..."
    if ! command -v rathole &> /dev/null; then
        print_error "rathole command not found. Please install rathole first."
        exit 1
    fi
    
    local keys_output
    keys_output=$(rathole --genkey 2>/dev/null || {
        print_error "Failed to generate keys. Make sure rathole is properly installed."
        exit 1
    })
...
create_server_config() {
    ...
    local remote_public_key="$4"
    local client_port="$5"
    local protocol="$6"
    local nodelay="$7"
    
    # Generate keys for server
    print_info "Generating keys for server..."
    local keys_output
...
create_client_config() {
    ...
    print_info "Generating keys for client..."
    local keys_output
    keys_output=$(generate_keys)
    echo "$keys_output"
    echo ""
...
main() {
    ...
    case "$type" in
        "server")
            if [ $# -ne 7 ]; then
                print_error "Server configuration requires exactly 7 parameters"
                show_usage
                exit 1
            fi
            ...
            create_server_config "$name" "$port" "$default_token" "$remote_public_key" "$client_port" "$protocol" "$nodelay"
            ;;
            
        "client")
            if [ $# -ne 7 ]; then
                print_error "Client configuration requires exactly 7 parameters"
                show_usage
                exit 1
            fi
            ...
            create_client_config "$name" "$remote_addr" "$default_token" "$remote_public_key" "$client_port" "$protocol" "$nodelay"
            ;;
